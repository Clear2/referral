...
// QueryReferral chains the current query on the "referral" edge.
func (_q *CreditTransactionQuery) QueryReferral() *ReferralQuery {
...
func (_q *CreditTransactionQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *CreditTransactionQuery) FirstIDX(ctx context.Context) int {
...
func (_q *CreditTransactionQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *CreditTransactionQuery) OnlyIDX(ctx context.Context) int {
...
func (_q *CreditTransactionQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (_q *CreditTransactionQuery) IDsX(ctx context.Context) []int {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *CreditTransactionQuery) Clone() *CreditTransactionQuery {
	...
	return &CreditTransactionQuery{
		...
		withReferral: _q.withReferral.Clone(),
...
// WithReferral tells the query-builder to eager-load the nodes that are connected to
// the "referral" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *CreditTransactionQuery) WithReferral(opts ...func(*ReferralQuery)) *CreditTransactionQuery {
	query := (&ReferralClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withReferral = query
	return _q
}
...
//		GroupBy(credittransaction.FieldCreateTime).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.CreditTransaction.Query().
//		Select(credittransaction.FieldCreateTime).
//		Scan(ctx, &v)
...
func (_q *CreditTransactionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*CreditTransaction, error) {
	...
	if query := _q.withReferral; query != nil {
		if err := _q.loadReferral(ctx, query, nodes, nil,
			func(n *CreditTransaction, e *Referral) { n.Edges.Referral = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *CreditTransactionQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*CreditTransaction, init func(*CreditTransaction), assign func(*CreditTransaction, *User)) error {
	...
	nodeids := make(map[int][]*CreditTransaction)
	for i := range nodes {
		fk := nodes[i].UserID
...
func (_q *CreditTransactionQuery) loadReferral(ctx context.Context, query *ReferralQuery, nodes []*CreditTransaction, init func(*CreditTransaction), assign func(*CreditTransaction, *Referral)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*CreditTransaction)
	for i := range nodes {
		fk := nodes[i].ReferralID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "referral_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *CreditTransactionQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(credittransaction.Table, credittransaction.Columns, sqlgraph.NewFieldSpec(credittransaction.FieldID, field.TypeInt))
	_spec.From = _q.sql
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withReferral != nil {
			_spec.Node.AddColumnOnce(credittransaction.FieldReferralID)
		}
	}
