...
func (_q *PermissionQuery) QueryGroup() *PermissionGroupQuery {
...
func (_q *PermissionQuery) QueryApis() *APIResourceQuery {
	query := (&APIResourceClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *PermissionQuery) QueryMenus() *MenuQuery {
...
func (_q *PermissionQuery) WithGroup(opts ...func(*PermissionGroupQuery)) *PermissionQuery {
	query := (&PermissionGroupClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PermissionQuery) WithApis(opts ...func(*APIResourceQuery)) *PermissionQuery {
	query := (&APIResourceClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PermissionQuery) WithMenus(opts ...func(*MenuQuery)) *PermissionQuery {
	query := (&MenuClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PermissionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Permission, error) {
	...
	if query := _q.withApis; query != nil {
		if err := _q.loadApis(ctx, query, nodes,
			func(n *Permission) { n.Edges.Apis = []*APIResource{} },
			func(n *Permission, e *APIResource) { n.Edges.Apis = append(n.Edges.Apis, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *PermissionQuery) loadGroup(ctx context.Context, query *PermissionGroupQuery, nodes []*Permission, init func(*Permission), assign func(*Permission, *PermissionGroup)) error {
...
func (_q *PermissionQuery) loadApis(ctx context.Context, query *APIResourceQuery, nodes []*Permission, init func(*Permission), assign func(*Permission, *APIResource)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Permission)
	nids := make(map[int]map[*Permission]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(permission.ApisTable)
		s.Join(joinT).On(s.C(apiresource.FieldID), joinT.C(permission.ApisPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(permission.ApisPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(permission.ApisPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
	...
	neighbors, err := withInterceptors[[]*APIResource](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
...
func (_q *PermissionQuery) loadMenus(ctx context.Context, query *MenuQuery, nodes []*Permission, init func(*Permission), assign func(*Permission, *Menu)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Permission)
	nids := make(map[int]map[*Permission]struct{})
	for i, node := range nodes {
