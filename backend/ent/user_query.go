...
type UserQuery struct {
	...
	withReceivedReferral   *ReferralQuery
...
// QueryReceivedReferral chains the current query on the "received_referral" edge.
func (_q *UserQuery) QueryReceivedReferral() *ReferralQuery {
	query := (&ReferralClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
...
func (_q *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withReceivedReferral:   _q.withReceivedReferral.Clone(),
...
// WithReceivedReferral tells the query-builder to eager-load the nodes that are connected to
// the "received_referral" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *UserQuery) WithReceivedReferral(opts ...func(*ReferralQuery)) *UserQuery {
	query := (&ReferralClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withReceivedReferral = query
	return _q
}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withReceivedReferral; query != nil {
		if err := _q.loadReceivedReferral(ctx, query, nodes, nil,
			func(n *User, e *Referral) { n.Edges.ReceivedReferral = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *UserQuery) loadReceivedReferral(ctx context.Context, query *ReferralQuery, nodes []*User, init func(*User), assign func(*User, *Referral)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Referral(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.ReceivedReferralColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
