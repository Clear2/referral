...
// HasReceivedReferral applies the HasEdge predicate on the "received_referral" edge.
func HasReceivedReferral() predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
...
// HasReceivedReferralWith applies the HasEdge predicate on the "received_referral" edge with a given conditions (other predicates).
func HasReceivedReferralWith(preds ...predicate.Referral) predicate.User {
	return predicate.User(func(s *sql.Selector) {
		step := newReceivedReferralStep()
		sqlgraph.HasNeighborsWith(s, step, func(s *sql.Selector) {
			for _, p := range preds {
				p(s)
			}
