...
func (_q *MenuQuery) QueryPermissions() *PermissionQuery {
...
func (_q *MenuQuery) WithPermissions(opts ...func(*PermissionQuery)) *MenuQuery {
	query := (&PermissionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *MenuQuery) loadPermissions(ctx context.Context, query *PermissionQuery, nodes []*Menu, init func(*Menu), assign func(*Menu, *Permission)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Menu)
	nids := make(map[int]map[*Menu]struct{})
	for i, node := range nodes {
